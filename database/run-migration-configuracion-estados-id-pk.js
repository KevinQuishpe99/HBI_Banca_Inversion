/**
 * PK numérica en configuracion_estados; codigo → valor_tramite.
 * Uso: npm run db:migrate-configuracion-estados-id-pk
 */
...
async function main() {
  ...
  try {
    const sqlPath = path.join(__dirname, 'migration-configuracion-estados-id-pk.sql');
    const sql = fs.readFileSync(sqlPath, 'utf8');
    await client.query(sql);
    console.log('OK: migration-configuracion-estados-id-pk aplicada.');
  } finally {
    client.release();
    await pool.end();
  }
