/**
 * Limpieza de DB + PK numérica en area_config.
 * Uso: node database/run-migration-cleanup-and-area-pk.js
 */
...
async function main() {
  ...
  try {
    const sqlPath = path.join(__dirname, 'migration-cleanup-and-area-pk.sql');
    const sql = fs.readFileSync(sqlPath, 'utf8');
    await client.query(sql);
    console.log('OK: migration-cleanup-and-area-pk aplicada.');
  } finally {
    client.release();
    await pool.end();
  }
