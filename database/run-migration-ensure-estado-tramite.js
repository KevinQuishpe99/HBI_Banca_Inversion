/**
 * Crea enum estado_tramite y alinea la columna tramites.estado.
 * Uso: node database/run-migration-ensure-estado-tramite.js
 */
...
async function main() {
  ...
  try {
    const sqlPath = path.join(__dirname, 'migration-ensure-estado-tramite.sql');
    const sql = fs.readFileSync(sqlPath, 'utf8');
    await client.query(sql);
    console.log('OK: migration-ensure-estado-tramite aplicada.');
  } finally {
    client.release();
    await pool.end();
  }
