/**
 * Aplica database/migration-area-config-supervisor-nullable.sql
 * Uso: npm run db:migrate-area-supervisor-nullable
 */
...
async function main() {
  ...
  try {
    const sqlPath = path.join(__dirname, 'migration-area-config-supervisor-nullable.sql');
    const sql = fs.readFileSync(sqlPath, 'utf8');
    await client.query(sql);
    console.log('OK: supervisor_id en area_config puede ser NULL (crear área sin equipo).');
  } finally {
    client.release();
    await pool.end();
  }
