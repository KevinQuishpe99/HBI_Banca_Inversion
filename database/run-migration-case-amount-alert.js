/**
 * Monto en trámites + umbral de alerta + notificación CASE_HIGH_AMOUNT.
 * Uso: npm run db:migrate-case-amount-alert
 */
...
async function main() {
  ...
  try {
    const sqlPath = path.join(__dirname, 'migration-case-amount-alert.sql');
    const sql = fs.readFileSync(sqlPath, 'utf8');
    await client.query(sql);
    console.log('OK: migration-case-amount-alert aplicada.');
  } finally {
    client.release();
    await pool.end();
  }
