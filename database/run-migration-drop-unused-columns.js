/**
 * Elimina columnas sin uso en toda la base de datos.
 * Uso: node database/run-migration-drop-unused-columns.js
 */
...
async function main() {
  ...
  try {
    const sqlPath = path.join(__dirname, 'migration-drop-unused-columns.sql');
    const sql = fs.readFileSync(sqlPath, 'utf8');
    await client.query(sql);
    console.log('OK: migration-drop-unused-columns aplicada.');
  } finally {
    client.release();
    await pool.end();
  }
