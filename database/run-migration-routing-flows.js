/**
 * Enrutado por flujos (supervisión vs directo a Legal).
 * Uso: npm run db:migrate-routing-flows
 */
...
async function main() {
  ...
  try {
    const sqlPath = path.join(__dirname, 'migration-routing-flows.sql');
    const sql = fs.readFileSync(sqlPath, 'utf8');
    await client.query(sql);
    console.log('OK: migration-routing-flows aplicada (routing_creator_area + columnas en cases).');
  } finally {
    client.release();
    await pool.end();
  }
