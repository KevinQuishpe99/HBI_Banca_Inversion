...
import { Eye, Loader2, Upload, FileText } from 'lucide-react';
...
export function Fase1DocumentosPanel({ operacionId }: Props) {
  ...
  const [docSeleccionado, setDocSeleccionado] = useState<DocumentoContractual | null>(null);
  ...
  return (
    <>
      <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Cargue la base contractual completa. Puede visualizar cada documento en el visor integrado
            (PDF, imágenes o vista demo HTML).
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <ul className="divide-y divide-slate-100 rounded-lg border border-slate-100">
            {docs?.length === 0 ? (
              ...
              docs?.map((d) => (
                <li key={d.id} className="flex items-start gap-3 p-3 text-sm hover:bg-slate-50/80">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-500">
                      ...
                      {typeof d.datosExtraidos?.subidoPor === 'string'
                        ? ` · Subido por ${d.datosExtraidos.subidoPor}`
                        : ''}
                      {d.tamanoBytes ? ` · ${(d.tamanoBytes / 1024).toFixed(1)} KB` : ''}
                    </p>
      ...
      <VisorDocumentoModal documento={docSeleccionado} onCerrar={() => setDocSeleccionado(null)} />
    </>
  );
}
